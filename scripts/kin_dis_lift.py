...
class Kinect_depth(object):
	...
	def determine_pick_coordinate(self):
		...
		x_co = 0.18*self.x -37.7 + self.x_constant
		y_co = -0.159*self.y + 75.9 +self.y_constant
		z_co = 31
		...
		if ( x_co > self.k ):
			...
			with open("pick_coordinates.dat", "w+") as f: 
				f.write(str(("pick_coordinate",x_co, y_co, z_co, 2 , "pick_coordinate")))  ## right pick
			print(pick_coordinate)
			...
			pick_coordinate = [x_co,y_co,z_co]
			with open("pick_coordinates.dat", "w+") as f: 
				f.write(str(("pick_coordinate",x_co, y_co, z_co, 1 , "pick_coordinate")))  ## right pick
			print(pick_coordinate)
	...
	def determine_place_coordinate(self):
		...
		if ( x_co > self.k ):
			...
			with open("place_coordinates.dat", "w+") as f: 
				f.write(str(("place_coordinate",x_co, y_co, z_co, 2 , "place_coordinate")))  ## right pick
			print(pick_coordinate)
			...
			pick_coordinate = [x_co,y_co,z_co]
			with open("place_coordinates.dat", "w+") as f: 
				f.write(str(("place_coordinate",x_co, y_co, z_co, 1 , "place_coordinate")))  ## right pick
			print(pick_coordinate)
